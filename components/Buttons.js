...
export function ThemeSwitch() {
  return (
    <ButtonBase id="theme-toggle" extra="p-2 flex justify-center">
      <p id="theme-text" className="hidden px-2">
        Switch theme
      </p>
      <svg
  ...
  return (
    <>
      <Link id="aboutme-button" href="#target-cv">
        <ButtonBase extra="w-28 sm:w-32 h-10 sm:h-12 mx-4">
          {" "}
          About Me{" "}
        </ButtonBase>
      </Link>
...
export function SelectButton({ type }) {
  if (type == "/") {
    ...
    return (
      <Link href="/">
        <ButtonBase id="go-home" extra="w-32 h-8">
          {" "}
          Go Back{" "}
        </ButtonBase>
      </Link>
