  ...
  return (
    <li key={job.id}>
      ...
      <div className="mt-2 ml-4 mb-6">
        <h1 className="mb-1.5 text-lg sm:text-xl font-semibold">{job.title}</h1>
        <p className="my-2 text-xs sm:text-sm text-right">{job.where}</p>
...
function SkillsSection({ props, tab }) {
  ...
  const levelsbase =
    "project-tag inline-block rounded-full font-semibold m-2.5 hover:scale-110";
...
export function TabWrapper({ props }) {
  const tabs_class =
    "inline-block text-center text-sm sm:text-base select-none cursor-pointer grow py-2.5 mx-3 border-b-2 backdrop-blur-sm backdrop-opacity-25";
