...
pdf.setProperties({
  ...
  subject:
    "Short written summary of a person's career, qualifications, and education",
  author: "Marco Ramos",
...
let contactHeight = 22;

pdf
  .setFontSize(24)
  .setFont(fontFamily, "bold")
  .setTextColor(darkColor)
  .text(cvdata.full_name, pageWidth / 2, 15, { align: "center" })
  .setFontSize(10)
  .setFont(fontFamily, "italic")
  .text(cvdata.email.secondary, firstContact, contactHeight, {
    align: "center",
  });
...
pdf.setDrawColor(lineColor).setLineWidth(0.5);
/*
  // horizontal line
...
pdf
  .setFontSize(14)
  .setFont(fontFamily, "bold")
  .text("Profile", 10, currentHeight, { align: "left" });

pdf
  .setLineWidth(0.1)
  .line(30, currentHeight - 1.5, pageWidth - 10, currentHeight - 1.5);

pdf
  .setFontSize(10)
  .setFont(fontFamily, "normal")
  .text(cvdata.introduction[0], 10, (currentHeight = currentHeight + 7), {
    align: "justify",
    maxWidth: textWidth,
  });
...
pdf
  .setLineWidth(0.1)
  .line(41, currentHeight - 1.5, pageWidth - 10, currentHeight - 1.5);
...
cvdata.timeline
  ...
  .map((time, index) => {
    ...
    pdf
      .setFontSize(11)
      .setFont(fontFamily, "bold")
      .text(time.title, 10, currentHeight, { align: "left" })
      .setFontSize(10)
      .setFont(fontFamily, "italic")
      .text(
      ...
      .text(time.where, rightSideEnd, currentHeight, { align: "right" })
      .setFont(fontFamily, "normal");
    ...
    time.description.map((desc, subindex) => {
      pdf
        .setFontSize(10)
        .text("•", 10, currentHeight, { align: "left" })
        .text(desc, 12, currentHeight, {
          align: "justify",
...
pdf
  .setFontSize(14)
  .setFont(fontFamily, "bold")
  .text("Education", 10, (currentHeight = currentHeight + 3), {
    align: "left",
  });

pdf
  .setLineWidth(0.1)
  .line(39, currentHeight - 1.5, pageWidth - 10, currentHeight - 1.5);

// currentHeight = currentHeight+7

cvdata.timeline
  .filter((item) => item.type === "education" && item.include)
  .map((time, index) => {
    pdf
      .setFontSize(10)
      .setFont(fontFamily, "bold")
      .text(time.title, 10, (currentHeight = currentHeight + 6), {
        align: "left",
      })
      .setFontSize(10)
      .setFont(fontFamily, "italic")
      .text(time.end, 10, (currentHeight = currentHeight + 6), {
        align: "left",
      })
      .text(time.where, rightSideEnd, currentHeight, { align: "right" })
      .setFont(fontFamily, "normal");
...
pdf
  .setFontSize(14)
  .setFont(fontFamily, "bold")
  .text("Skills", 10, (currentHeight = currentHeight + 9), { align: "left" });

pdf
  .setLineWidth(0.1)
  .line(28, currentHeight - 1.5, pageWidth - 10, currentHeight - 1.5);
...
pdf
  .setFontSize(10)
  .setFont(fontFamily, "normal")
  .text(skillstring, 10, currentHeight + 7, {
    align: "justify",
    maxWidth: textWidth - 2,
  });
...
pdf
  ...
  .setFont(fontFamily, "italic")
  .setTextColor("#8a8a8a")
  .textWithLink(footerText, pageWidth / 2, pageHeight - 5, {
    align: "center",
